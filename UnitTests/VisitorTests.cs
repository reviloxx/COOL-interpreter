...
// Currently it is only tested if building the AST and executing it does not throw an exception.
// It is challenging to test if the execution itself was done right - one way to improve it would be to validate if the expected console output was produced.
public class VisitorTests : VisitorTestsBase
{
    ...
    {
        var file = FileHelper.GetFile(TestType.Algorithm, testCase);
    ...
    {
        var file = FileHelper.GetFile(TestType.Parsing, testCase);
        var context = GetProgramContext(file);
    ...
    {
        var file = FileHelper.GetFile(TestType.Semantics, testCase);
        var context = GetProgramContext(file);
